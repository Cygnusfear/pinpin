...
export class UrlWidgetFactory implements WidgetFactory<UrlWidget> {
  ...
  canHandle(data: any): boolean {
    if (typeof data === "string") {
      ...
      if (
        ...
        try {
          const _url = new URL(`https://${trimmed}`);
          return true;
        } catch {
          return false;
        }
  ...
  async create(
    ...
    const defaultData = genericFactory.getDefaultWidgetData("url", position, {
      width: 300,
      height: 200,
    });

    // Merge with URL-specific data
    const widgetData: WidgetCreateData<UrlWidget> = {
      ...defaultData,
      url,
