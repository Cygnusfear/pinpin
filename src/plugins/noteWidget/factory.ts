...
export class NoteWidgetFactory implements WidgetFactory<NoteWidget> {
  ...
  async create(
    ...
    if (typeof data === 'string') {
      ...
      textColor = data.textColor || this.getContrastingTextColor(backgroundColor);
  ...
  private calculateNoteSize(content: string): { width: number; height: number } {
    const minWidth = 150;
    const minHeight = 100;
    const maxWidth = 400;
    const maxHeight = 600;
  ...
  updateNoteContent(widget: NoteWidget, newContent: string): Partial<NoteWidget> {
  ...
  changeNoteColor(widget: NoteWidget, backgroundColor: string): Partial<NoteWidget> {
    return {
      backgroundColor,
      textColor: this.getContrastingTextColor(backgroundColor),
    };
