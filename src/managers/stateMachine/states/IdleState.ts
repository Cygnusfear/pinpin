...
export class IdleState extends BaseState {
  ...
  onMouseDown(event: Extract<StateMachineEvent, { type: 'mousedown' }>): StateTransition | null {
    ...
    if (button === 0) {
      if (hitWidget) {
        ...
        const transformHandle = this.getTransformHandleAtPoint(point, hitWidget);
        if (transformHandle) {
  ...
  onWheel(event: Extract<StateMachineEvent, { type: 'wheel' }>): StateTransition | null {
    ...
    if (event.modifiers.ctrl || event.modifiers.meta) {
      ...
      const newScale = Math.max(0.01, Math.min(100, this.context.canvasTransform.scale * zoomFactor));
      
      // Calculate new offset to zoom towards the screen point
      const scaleRatio = newScale / this.context.canvasTransform.scale;
      const newTransform = {
        x: event.screenPoint.x - (event.screenPoint.x - this.context.canvasTransform.x) * scaleRatio,
        y: event.screenPoint.y - (event.screenPoint.y - this.context.canvasTransform.y) * scaleRatio,
        scale: newScale,
      };
  ...
  private handleWidgetSelection(widget: import('../../../types/canvas').Widget, modifiers: import('../../../types/canvas').KeyModifiers): void {
  ...
  private getTransformHandleAtPoint(point: { x: number; y: number }, widget: import('../../../types/canvas').Widget): import('../types').TransformHandle | null {
    ...
    const handles = [
      { type: 'resize' as const, position: 'nw' as const, x: widget.x - handleOffset, y: widget.y - handleOffset, cursor: 'nw-resize' },
      { type: 'resize' as const, position: 'n' as const, x: widget.x + widget.width / 2 - handleOffset, y: widget.y - handleOffset, cursor: 'n-resize' },
      { type: 'resize' as const, position: 'ne' as const, x: widget.x + widget.width - handleOffset, y: widget.y - handleOffset, cursor: 'ne-resize' },
      { type: 'resize' as const, position: 'e' as const, x: widget.x + widget.width - handleOffset, y: widget.y + widget.height / 2 - handleOffset, cursor: 'e-resize' },
      { type: 'resize' as const, position: 'se' as const, x: widget.x + widget.width - handleOffset, y: widget.y + widget.height - handleOffset, cursor: 'se-resize' },
      { type: 'resize' as const, position: 's' as const, x: widget.x + widget.width / 2 - handleOffset, y: widget.y + widget.height - handleOffset, cursor: 's-resize' },
      { type: 'resize' as const, position: 'sw' as const, x: widget.x - handleOffset, y: widget.y + widget.height - handleOffset, cursor: 'sw-resize' },
      { type: 'resize' as const, position: 'w' as const, x: widget.x - handleOffset, y: widget.y + widget.height / 2 - handleOffset, cursor: 'w-resize' },
      { type: 'rotate' as const, position: 'rotation' as const, x: widget.x + widget.width / 2 - handleOffset, y: widget.y - 30, cursor: 'grab' },
    ];
  ...
  private handleTransformHandleClick(handle: import('../types').TransformHandle, point: { x: number; y: number }): StateTransition {
  ...
  private getTransformOrigin(handle: import('../types').TransformHandle): { x: number; y: number } {
  ...
  private getCursorForPoint(point: { x: number; y: number }, hitWidget?: import('../../../types/canvas').Widget): string {
