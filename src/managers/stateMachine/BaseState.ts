...
export abstract class BaseState {
  ...
  onEnter(_fromState?: InteractionStateName): void {
    // Default implementation - update cursor
    this.callbacks.onCursorChange(this.cursor);
  }

  onExit(_toState?: InteractionStateName): void {
    // Default implementation - cleanup if needed
  }

  onMouseDown(
    _event: Extract<StateMachineEvent, { type: "mousedown" }>,
  ): StateTransition | null {
    return null; // No transition by default
  }

  onMouseMove(
    _event: Extract<StateMachineEvent, { type: "mousemove" }>,
  ): StateTransition | null {
    return null;
  }
  ...
  onKeyUp(
    _event: Extract<StateMachineEvent, { type: "keyup" }>,
  ): StateTransition | null {
    return null;
  }

  onWheel(
    _event: Extract<StateMachineEvent, { type: "wheel" }>,
  ): StateTransition | null {
    return null;
  }

  onContextMenu(
    _event: Extract<StateMachineEvent, { type: "contextmenu" }>,
  ): StateTransition | null {
    return null;
  }
  ...
  protected getWidgetAtPoint(point: {
    x: number;
    y: number;
  }): import("../../types/canvas").Widget | undefined {
    // Find the topmost widget at the given point
    const sortedWidgets = [...this.context.widgets].sort(
  ...
  protected constrainToAxes(delta: { x: number; y: number }): {
    x: number;
    y: number;
  } {
