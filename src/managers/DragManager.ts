...
export class DragManager {
  ...
  private updateMultipleWidgetTransforms: (
    updates: Array<{
      id: string;
      transform: {
        x?: number;
  ...
  constructor(
    ...
    this.updateMultipleWidgetTransforms =
      updateMultipleWidgetTransforms || (() => {});
  }
  ...
  updateDrag(currentPosition: Point, modifiers: KeyModifiers): void {
    if (!this.dragState.isDragging) return;
  ...
  protected updateWidgetPositions(delta: Point): void {
    const updates = this.dragState.draggedWidgetIds
      .map((id) => {
  ...
  private applySnapping(delta: Point): {
    delta: Point;
  ...
  isPointBeingDragged(point: Point): boolean {
    ...
    const distance = Math.sqrt(
      (point.x - this.dragState.currentPosition.x) ** 2 +
        (point.y - this.dragState.currentPosition.y) ** 2,
    );
...
export class DragPerformanceMonitor {
  ...
  getMetrics() {
    return {
      ...this.metrics,
      maxUpdateTime:
        this.metrics.updateTimes.length > 0
          ? Math.max(...this.metrics.updateTimes)
          : 0,
      minUpdateTime:
        this.metrics.updateTimes.length > 0
          ? Math.min(...this.metrics.updateTimes)
          : 0,
      averageDataPerUpdate:
        this.metrics.updateCount > 0
          ? this.metrics.totalDataSynced / this.metrics.updateCount
          : 0,
    };
  }
